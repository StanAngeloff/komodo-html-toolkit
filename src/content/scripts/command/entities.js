(function(){
  ...
  var entities = ['quot', // quotation mark, U+0022 ISOnum
  ...
  $self.onViewOpened = function onViewOpened(e) {
    var view;
    view = e.originalTarget;
    return view && view.scimoz ? view.scimoz.registerImage(ACIID_HTML_ENTITY, ko.markers.getPixmap('chrome://htmltoolkit/skin/images/ac_html_entity.xpm')) : null;
  };
  $self.controller = function controller() {
    var canChangeTriggerKeys, command, supportedLanguages, triggerKeys;
    root.command.language.controller.apply(this, [(command = 'entities'), (triggerKeys = '&'), (supportedLanguages = ['HTML']), (canChangeTriggerKeys = false)]);
    this.trigger = function trigger(e) {
      return this.onKeyEvent('up', function() {
        var __a, __b, __c, entity, lineRange, lineStartPosition, scimoz;
        ...
        if (scimoz.currentPos === scimoz.anchor && scimoz.currentPos > 0 && scimoz.getStyleAt(scimoz.currentPos - 1) === scimoz.SCE_UDL_M_ENTITY) {
          ...
          return /[^&]&$/.test(lineRange) ? scimoz.autoCShow(1, ((function() {
            __a = []; __b = entities;
            for (__c = 0; __c < __b.length; __c++) {
              entity = __b[__c];
              __a.push('&' + entity + ';?' + ACIID_HTML_ENTITY);
            }
            return __a;
    ...
    return root.trapExceptions(this);
  };
